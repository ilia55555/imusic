...
async function start() {
  ...
  function detect() {
    ...
    const noteLogContainer = document.getElementById("note-log");

    if (freq !== -1) {
      const note = getNoteFromFrequency(freq);
      liveNote.innerText = `در حال تشخیص: ${note}`;

      if (note !== lastNote) {
        noteLog.push(note);
        lastNote = note;
        noteLogContainer.innerText = noteLog.join("  -  ");
      }
    } else {
      liveNote.innerText = "در حال تشخیص: --";
    }
